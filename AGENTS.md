...
- The reusable event debug server package now lives in `pkgs/droner/dronerd/eventdebug`; `cmd/eventdebug` is the standalone wrapper and the bundled CLI entrypoint is `droner debugger`.
...
- New session creation now goes through `session.enrichment.requested|succeeded|failed` before environment provisioning. `session_projection.branch` and `worktree_path` are nullable until enrichment succeeds, and branch uniqueness is enforced only for live sessions via a partial index rather than globally across completed/deleted history.
