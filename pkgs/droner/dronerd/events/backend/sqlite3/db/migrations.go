...
func NewMigrationProvider(conn *sql.DB) (*goose.Provider, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("open sqlite3 db migrations: %w", err)
	}
	provider, err := goose.NewProvider(goose.DialectSQLite3, conn, files)
	if err != nil {
		return nil, fmt.Errorf("create sqlite3 db migration provider: %w", err)
	}
	return provider, nil
}
...
func EnsureMigrations(ctx context.Context, conn *sql.DB) error {
	...
	if err != nil {
		return fmt.Errorf("get current sqlite3 db migration version: %w", err)
	}
	...
	if _, err := provider.Up(ctx); err != nil {
		return fmt.Errorf("run sqlite3 db migrations: %w", err)
	}
	return nil
}
