...
func (sm *subscriptionManager) subscribe(ctx context.Context, remoteURL string, branch string, logger *slog.Logger, onDelete func(sessionID string)) error {
	...
	if _, exists := sm.subs[key]; exists {
		logger.Info("Remote subscription already active",
	...
	handler := func(event remote.BranchEvent) {
		logger.Info("Remote event received",
		...
		case remote.PRClosed, remote.PRMerged, remote.BranchDeleted:
			logger.Info("Triggering session cleanup due to remote event",
				"event_type", event.Type,
				"branch", event.Branch,
				"pr_number", event.PRNumber,
			)
		...
		default:
			logger.Debug("Ignoring non-triggering event",
				"event_type", event.Type,
				"branch", event.Branch,
			)
		}
	...
	logger.Info("Remote subscription started",
		"remote_url", remoteURL,
		"branch", branch,
	)
...
func (sm *subscriptionManager) unsubscribe(ctx context.Context, remoteURL string, branch string, logger *slog.Logger) error {
	key := remoteURL + ":" + branch
	...
	if _, exists := sm.subs[key]; !exists {
		logger.Info("Remote subscription not found, skipping unsubscribe",
			"remote_url", remoteURL,
			"branch", branch,
		)
	...
	logger.Info("Remote subscription stopped",
		"remote_url", remoteURL,
		"branch", branch,
	)
